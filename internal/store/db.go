...
type Store struct {
	DB              *sql.DB
	edgeMu          sync.Mutex
	limitationCache map[int64][]*model.LimitationClause
}
...
func Open(path string) (*Store, error) {
	...
	db.SetMaxOpenConns(1) // modernc sqlite 单连接最稳，避免跨连接事务问题
	...
	return &Store{DB: db, limitationCache: map[int64][]*model.LimitationClause{}}, nil
}
